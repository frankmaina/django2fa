...
def index(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			#process code
			verification_session = PhoneVerification.objects.get(user=user,active=1,verified=0)
			if str(code) == str(verification_session.code):
				#the code passed hence ther verification request is not active but is verified
				...
				#the code failed hence ther verification request is not active and also not verified
				verification_session.active=0
				verification_session.verified=0
				verification_session.save()
				#we logout the user so that he/she would have to submit another phone verification request
				logout(request)
...
def email_verification(request):
	if request.GET['token']:
		...
		token = str(token)
		try:
			email_verification = EmailVerification.objects.get(email_token = token,active=1,verified=0)
			form = signinform()
